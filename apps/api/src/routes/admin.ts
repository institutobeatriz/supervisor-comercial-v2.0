...
function checkAdminAuth(request: any, reply: any) {
  const adminKey = process.env.ADMIN_API_KEY;
  const isDev = process.env.NODE_ENV === 'development';
  if (!adminKey) {
    if (isDev) return true;
    reply.status(503).send({ error: 'ADMIN_API_KEY not configured' });
    return false;
  }
...
const adminRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/reports/weekly', {
    ...
    const report = await getWeeklyReport(sellerId || '', startDate, endDate);
  ...
  fastify.post('/rag/search', {
    ...
    const chunks = await findSimilarRagChunks(
      body.embedding,
      body.objection,
      body.limit || 5
    );
