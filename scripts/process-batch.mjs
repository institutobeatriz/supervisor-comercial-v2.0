...
async function reprocessBatch() {
  console.log('🔄 Processando lote pequeno...\n');
  ...
  if (result.rowCount === 0) {
    console.log('✅ Todas as mensagens foram classificadas!');
    await pool.end();
    process.exit(0);
  }
  
  for (let i = 0; i < result.rows.length; i++) {
    const msg = result.rows[i];
  ...
  console.log(`\n⏳ Aguarde 10 segundos para o próximo lote...`);
  
  await pool.end();
  process.exit(0);
}

reprocessBatch().catch(console.error);
