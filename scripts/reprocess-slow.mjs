...
async function reprocessSlowly() {
  ...
  console.log(`Processando ${result.rowCount} mensagens (lote pequeno)...\n`);
  ...
  for (const msg of result.rows) {
    await classifyQueue.add('classify', {
      ...
      delay: 2000 // 2 segundos entre cada job
    });
    console.log(`  ✓ ${msg.text.substring(0, 50)}...`);
  }
  
  console.log(`\n✅ ${result.rowCount} jobs adicionados à fila`);
  
  await pool.end();
  process.exit(0);
}

reprocessSlowly().catch(console.error);
