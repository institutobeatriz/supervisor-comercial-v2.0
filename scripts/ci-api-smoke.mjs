  ...
  },
  {
    ...
    validators: [
      ({ data }) => {
        ...
        // phase43 enforcement: legacyFallbackState must be 'disabled' in all smoke runs
        const legacyState = data?.producer?.summary?.legacyFallbackState;
        pushIf(errors, !legacyState || legacyState === 'disabled', `producer.summary.legacyFallbackState must be disabled (got=${legacyState || 'missing'}; phase43 enforcement)`);
        // phase43 enforcement: allowLegacyFallback must be false
