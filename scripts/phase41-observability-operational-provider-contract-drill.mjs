...
const SCRIPT = path.resolve(ROOT, 'scripts/phase41-observability-operational-provider-contract.mjs');
const DRILL_DIR = path.resolve(ROOT, 'logs/monitoring/phase41-drill');
...
async function main() {
  ...
  assert((passRun.status ?? 1) === 0, `phase41 materialized run should exit 0 (stderr=${passRun.stderr})`);
  ...
  assert(passReport?.operationalProvider?.mode === 'materialized_contract', `expected provider mode materialized_contract, got ${passReport?.operationalProvider?.mode}`);
  assert(passReport?.operationalProvider?.contractLoaded === true, 'expected contractLoaded=true');
  assert(passContract?.schema === 'fullcycle.observability.operational-provider.v1', `unexpected contract schema ${passContract?.schema}`);
  assert(passContract?.sources?.incidentAutomation?.incidentCount === 1, 'expected one incident automation record');
  assert(passAnalytics?.current?.operationalProvider?.mode === 'materialized_contract', 'expected analytics.current operationalProvider mode');
  ...
  assert((replayRun.status ?? 1) === 0, `phase41 contract replay should exit 0 (stderr=${replayRun.stderr})`);

  const replayReport = await readJson(backendReportFile);
  assert(replayReport?.operationalProvider?.contractLoaded === true, 'expected replay to use existing contract');
  assert(replayReport?.operationalProvider?.sources?.incidentAutomation?.loaded === true, 'expected contract-backed incidentAutomation source to remain loaded');
  ...
  assert((failRun.status ?? 0) !== 0, 'phase41 missing contract run should exit non-zero');

  console.log('[OK] phase41 operational provider contract materialize/replay/fail validated');
  console.log(`[OK] phase41 drill report=${backendReportFile}`);
}

main().catch((error) => {
  console.error(`Unexpected phase41 drill failure: ${error instanceof Error ? error.stack || error.message : String(error)}`);
  process.exit(1);
});
