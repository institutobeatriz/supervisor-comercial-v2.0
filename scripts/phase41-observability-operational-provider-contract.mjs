...
function renderProviderDashboard({ ts, contract, report }) {
  ...
  const lines = [
    ...
    `- Producer mode: ${provider.producerMode || 'n/a'}`,
    `- Producer ready: ${provider.producerReady === null || provider.producerReady === undefined ? 'n/a' : (provider.producerReady ? 'yes' : 'no')}`,
    `- Legacy fallback state: ${provider.legacyFallbackState || 'n/a'}`,
    '',
    '## Source Summary',
    '',
