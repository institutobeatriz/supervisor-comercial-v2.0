...
async function main() {
  ...
  assert(passStore?.oncall?.sourceMode === 'operational_contract', `expected operational source mode, got ${passStore?.oncall?.sourceMode}`);
  assert(passStore?.oncall?.operationalProvider?.contractLoaded === true, 'expected operational provider contract loaded');
